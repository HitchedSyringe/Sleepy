...
if TYPE_CHECKING:
    ...
    class _SizedImageConverter(Protocol):
...
class ImageResourceConverter(commands.Converter["DiscordImage"]):
    ...
    1. Image Attachments (returns a :class:`discord.Attachment`)
        a. This behaves similarly to the :class:`discord.Attachment`
           special converter, but performs extra checks to ensure the
           attachment passed is an image attachment.
        b. This also converts a replied message's attachments. If the
           context message is a reply and has image attachments, then
           those image attachments will be converted over the replied
    ...
    5. Image URLs (returns a :class:`DiscordMediaURL` denoting contents)
    ...
    async def convert(self, ctx: SleepyContext, argument: str) -> DiscordImage:
        try:
...
class ImageAttachment(app_commands.Transformer):
    ...
    transformer, but performs extra checks to ensure the attachment
    passed is an image attachment.
    ...
    async def transform(self, itn: Interaction, value: Any, /) -> Attachment:
        _validate_attachment(self, value)
        return value
...
def _validate_attachment(state: _SizedImageConverter, attachment: Attachment) -> None:
    mime = attachment.content_type
    if mime is None or "image/" not in mime:
        raise BadImageArgument(attachment)

    size = attachment.size
    max_size = state._max_size
    if max_size is not None and size > max_size:
        ...
        if hasattr(converter, "__sleepy_converters_is_image_attachment__"):
            ...
            if isinstance(res, list):
                for attachment in res:
                    _validate_attachment(converter, attachment)
            else:
                _validate_attachment(converter, res)
