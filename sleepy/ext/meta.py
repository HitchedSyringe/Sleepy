...
class SleepyHelpCommand(commands.HelpCommand):
    ...
    async def subcommand_not_found(self, command, string):
        subcmds = await self.filter_commands(command.commands, sort=True)

        if isinstance(command, commands.Group) and subcmds:
...
class Meta(commands.Cog):
    ...
    async def ping(self, ctx):
        ...
        delta = (msg_ts - datetime.utcnow()).total_seconds()
    ...
    async def serverinfo(self, ctx, *, guild=None):
        ...
        embed.add_field(
            ...
            value=(
                ...
                f"\n<:ar:862433028088135711> **Region:** {guild.region}"
        ...
        embed.add_field(
            ...
            value=f"**{plural(boosts, ',d'):boost}** \N{BULLET} {next_}"
                  f"\n\n0 {progress_bar(30, boosts, per=2)} 30",
            inline=False
        )
