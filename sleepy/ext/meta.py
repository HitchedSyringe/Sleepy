...
class Meta(commands.Cog):
    ...
    async def serverinfo(self, ctx, *, guild=None):
        ...
        embed = Embed(colour=0x2F3136, description=guild.description or "")
        embed.set_author(name=guild.name)

        if (icon := guild.icon) is not None:
            icon_url = icon.with_static_format("png")
            embed.description += f"\n**[Icon]({icon_url})**"
            embed.set_thumbnail(url=icon)

        if (banner := guild.banner) is not None:
            banner_url = banner.with_static_format("png")
            embed.description += f" \N{BULLET} **[Banner]({banner_url})**"
            embed.set_image(url=banner_url)

        if (d_spl := guild.discovery_splash) is not None:
            embed.description += f" \N{BULLET} **[Discovery Splash]({d_spl.with_format('png')})**"

        if (i_spl := guild.splash) is not None:
            embed.description += f" \N{BULLET} **[Invite Splash]({i_spl.with_format('png')})**"
